...
def generate_launch_description():
    ...
    robot_description_content = Command(
        [
            ...
            PathJoinSubstitution([ur3_pick_place_package, 'urdf', 'ur3_fixed.urdf.xacro'])
        ]
    )
    ...
    # Bridge between ROS and Gazebo - Using custom bridge node
    bridge = Node(
        ...
        arguments=[
            # Only the clock topic
            '/clock@rosgraph_msgs/msg/Clock[gz.msgs.Clock'
        ],
        output='screen'
    )

    # Trajectory visualizer for RViz
    ...
    # Add a static transform publisher to help with TF buffer warnings
    static_tf = Node(
        package='tf2_ros',
    ...
    return LaunchDescription([
        ...
        RegisterEventHandler(
            event_handler=OnProcessExit(
                target_action=spawn_entity,
                on_exit=[bridge, relay_node, static_tf]
            )
        ),
        # Event handlers for sequencing
        delay_rviz_after_gazebo,
        # Add visualization nodes after RViz starts
        RegisterEventHandler(
            event_handler=OnProcessExit(
                target_action=rviz_node,
                on_exit=[trajectory_visualizer, ur3_color_enhancer]
            )
        )
    ])
